...
def main():
    ...
    tmpdir = "%s/berycoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and berycoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    ...
    if "BERYCOIND" not in os.environ:
        os.environ["BERYCOIND"] = build_dir + '/src/berycoind' + exeext
        os.environ["BERYCOINCLI"] = build_dir + '/src/berycoin-cli' + exeext
...
class RPCCoverage(object):
    ...
    commands per `berycoin-cli help` (`rpc_interface.txt`).
