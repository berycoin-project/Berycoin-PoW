Berycoin Core version *0.15.1* is now available from:

  <https://download.berycoin.org/berycoin-0.15.1/>
...
A number of changes to the way Berycoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 beryflowers per kB (0.00001 BERY). This is to prepare for dropping the minimum transaction fee to 0.1 beryflowers per kB (0.0001 BERY) in the next release.
